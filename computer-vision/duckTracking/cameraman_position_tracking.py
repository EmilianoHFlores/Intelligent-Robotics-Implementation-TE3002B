...
class CameramanPositionTracker:
    def __init__(self, source, calibration_2d_points_file, calibration_3d_points_file, save_video=''):
        ...
        if save_video != '':
            ...
            self.out = cv2.VideoWriter(save_video, fourcc, self.FPS, (self.FRAME_WIDTH*2, self.FRAME_HEIGHT*2))
    ...
    def analyze_video(self):
        ...
        #self.prev_birds_eye_view = cv2.warpPerspective(prev_frame, self.homography, (self.FRAME_WIDTH, self.FRAME_HEIGHT))
        prev_useful_area = self.get_useful_area(prev_frame)
        prev_line_list = self.detect_lines(prev_frame)
        prev_line_image = self.show_only_lines(prev_frame, prev_line_list)
        ...
        while True:
            ...
            if not ret:
                break
            self.birds_eye_view = cv2.warpPerspective(frame, self.homography, (self.FRAME_WIDTH, self.FRAME_HEIGHT))
            useful_area = self.get_useful_area(frame)
            
            line_list = self.detect_lines(frame)
            line_image = self.show_only_lines(frame, line_list)
            ...
            angle, t = self.calculate_motion_essential(kp1, kp2, matches, self.FRAME_WIDTH, self.FRAME_HEIGHT)
            
            if np.isscalar(t[0]) or np.isscalar(t[1]):
                t_0 = [0]
                t_1 = [0]
            else:
                t_0 = t[0]
            ...
            self.cameraman_current_position = [self.cameraman_current_x + x_motion, self.cameraman_current_y + y_motion]
            self.cameraman_positions.append([x_motion, y_motion])
            ...
            self.birds_eye_show.set_data(cv2.cvtColor(self.birds_eye_view, cv2.COLOR_BGR2RGB))
            ...
            self.cameraman_position_plot.set_ydata([pos[1] for pos in self.cameraman_positions])
            # adjust the limits
            self.axs[4].set_xlim(min([pos[0] for pos in self.cameraman_positions]), max([pos[0] for pos in self.cameraman_positions]))
            self.axs[4].set_ylim(min([pos[1] for pos in self.cameraman_positions]), max([pos[1] for pos in self.cameraman_positions]))
            ...
            if self.save_video != '':
                show_img = imutils.resize(show_img, width=self.FRAME_WIDTH*2, height=self.FRAME_HEIGHT*2)
